...
func probeAURHelper(helper string) AURHelperStatus {
	...
	stdout, stderr, err := runCommandStdout(helper, "-Qua")
	...
	if exitErr, ok := err.(*exec.ExitError); ok && exitErr.ExitCode() == 1 && trimmedOut == "" {
		status.Working = true
	...
	if trimmedErr != "" {
		status.Details += " | " + oneLine(trimmedErr)
	}
	if trimmedOut != "" {
		status.Details += " | stdout: " + oneLine(trimmedOut)
	}
...
func getAURUpdates(aurHelper string) ([]string, error) {
	stdout, stderr, err := runCommandStdout(aurHelper, "-Qua")
	trimmedOut := strings.TrimSpace(stdout)
	...
	if err != nil {
		...
		if trimmedOut == "" {
			if strings.TrimSpace(stderr) != "" {
				return nil, fmt.Errorf("%s", oneLine(stderr))
			}
			return nil, err
		}
	}
...
func runCommandStdout(name string, args ...string) (string, string, error) {
	cmd := exec.Command(name, args...)
